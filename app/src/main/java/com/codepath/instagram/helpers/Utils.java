...
public class Utils {
    ...
    public static List<InstagramPost> fetchPosts(Context ctx, String fileName) {
        List<InstagramPost> posts = new ArrayList<>();
        try {
            JSONObject jsonPosts = loadJsonFromAsset(ctx, fileName);
            posts = decodePostsFromJsonResponse(jsonPosts);
        } catch (IOException e) {
            e.printStackTrace();
